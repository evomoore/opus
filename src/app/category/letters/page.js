...
export default function LettersPage() {
  ...
  // Fetch all letters articles (no pagination on API)
  useEffect(() => {
    setIsLoading(true);
    setError(null);
    const fetchArticles = async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}/articles?category=Letters`
        );
        ...
        setError("Failed to load letters articles");
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <main className="max-w-4xl mx-auto px-4 py-4">
        ...
        <nav className="mb-2" aria-label="Breadcrumb">
          <ol className="flex flex-wrap items-center text-sm text-gray-500 gap-1">
            ...
            <li className="truncate text-gray-800 font-semibold" aria-current="page">Letters</li>
          </ol>
        </nav>
        ...
        {isLoading ? (
          <div className="text-center text-gray-500 py-12">Loading letters articles...</div>
        ) : error ? (
          <div className="text-center text-red-500 py-12">{error}</div>
        ) : (
          <>
            {/* Grid of letters articles */}
              ...
              {paginatedArticles.map((article) => (
                ...
                >
                  {article.media?.featured_image?.url ? (
                    ...
                    <img
                      src="https://res.cloudinary.com/phonetag/image/upload/v1747421822/default-images/hlu8cde20ntizfjq0tbn.png"
                      alt="Default letters article cover"
