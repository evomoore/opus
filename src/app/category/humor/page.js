...
export default function HumorPage() {
  ...
  // Fetch all humor articles (no pagination on API)
  useEffect(() => {
    setIsLoading(true);
    setError(null);
    const fetchArticles = async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}/articles?category=Humor`
        );
        ...
        setError("Failed to load humor articles");
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <main className="max-w-4xl mx-auto px-4 py-4">
        ...
        <nav className="mb-2" aria-label="Breadcrumb">
          <ol className="flex flex-wrap items-center text-sm text-gray-500 gap-1">
            ...
            <li className="truncate text-gray-800 font-semibold" aria-current="page">Humor</li>
          </ol>
        </nav>
        <h1 className="text-3xl font-bold mb-4">Humor</h1>
        ...
        {isLoading ? (
          <div className="text-center text-gray-500 py-12">Loading humor articles...</div>
        ) : error ? (
          <div className="text-center text-red-500 py-12">{error}</div>
        ) : (
          <>
            {/* Grid of humor articles */}
              ...
              {paginatedArticles.map((article) => (
                ...
                >
                  {article.media?.featured_image?.url ? (
                    ...
                    <img
                      src="https://res.cloudinary.com/phonetag/image/upload/v1747356428/default-images/i05vwkjzgj8chahtqkz3.png"
                      alt="Default humor article cover"
