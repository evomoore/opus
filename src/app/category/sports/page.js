...
export default function SportsPage() {
  ...
  // Fetch all sports articles (no pagination on API)
  useEffect(() => {
    setIsLoading(true);
    setError(null);
    const fetchArticles = async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}/articles?category=Sports`
        );
      ...
      <main className="max-w-4xl mx-auto px-4 py-4">
        ...
        <nav className="mb-2" aria-label="Breadcrumb">
          <ol className="flex flex-wrap items-center text-sm text-gray-500 gap-1">
            ...
            <li className="truncate text-gray-800 font-semibold" aria-current="page">Sports</li>
          </ol>
        </nav>
        ...
        {isLoading ? (
          <div className="text-center text-gray-500 py-12">Loading sports articles...</div>
        ) : error ? (
              ...
              {paginatedArticles.map((article) => (
                ...
                >
                  {article.media?.featured_image?.url ? (
                    ...
                    <img
                      src="https://res.cloudinary.com/phonetag/image/upload/v1748047004/default-images/zb0d7nq9pmf8wp6e5ygj.png"
                      alt="Default sports article cover"
