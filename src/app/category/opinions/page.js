...
export default function OpinionsPage() {
  ...
  // Fetch all opinion articles (no pagination on API)
  useEffect(() => {
    setIsLoading(true);
    setError(null);
    const fetchArticles = async () => {
      try {
        const res = await fetch(
          `${API_BASE_URL}/articles?category=Opinions`
        );
        ...
        setError("Failed to load opinion articles");
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <main className="max-w-4xl mx-auto px-4 py-4">
        ...
        <nav className="mb-2" aria-label="Breadcrumb">
          <ol className="flex flex-wrap items-center text-sm text-gray-500 gap-1">
            ...
            <li className="truncate text-gray-800 font-semibold" aria-current="page">Opinions</li>
          </ol>
        </nav>
        <h1 className="text-3xl font-bold mb-4">Opinions</h1>
        ...
        {isLoading ? (
          <div className="text-center text-gray-500 py-12">Loading opinion articles...</div>
        ) : error ? (
          <div className="text-center text-red-500 py-12">{error}</div>
        ) : (
          <>
            {/* Grid of opinion articles */}
              ...
              {paginatedArticles.map((article) => (
                ...
                >
                  {article.media?.featured_image?.url ? (
                    ...
                    <img
                      src="https://res.cloudinary.com/phonetag/image/upload/v1748047003/default-images/vu9ivu50ra87z4d3n7jf.png"
                      alt="Default opinion article cover"
